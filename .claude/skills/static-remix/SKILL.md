...
- **Secondary canvas**: 9:16 vertical (1080×1920px) for Stories/Reels — only when user requests or
  when the brief calls for full-screen immersive layout; critical content in 1080×1350px safe zone
...
2. Inject the brand color lock into every prompt:
   ```
   ...
   ```
3. If the brief features any person/face, inject the people lock:
   ```
   ...
   ```
4. Aspect ratio: `"1:1"` for all feed ads (default). Use `"9:16"` only when the brief
   explicitly calls for Stories/Reels format or the user requests it.
5. Run:
```bash
